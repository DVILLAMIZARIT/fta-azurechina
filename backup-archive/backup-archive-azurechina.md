# FastTrack for Azure

See our [FastTrack for Azure landing page](https://github.com/Azure/FastTrackForAzure) for more information.


# Azure Backup and Archive

## Reference Resources

* [Reference Resources](articles/backup-archive-azurechina-reference-resources.md)
    * [Azure Backup Overview](articles/backup-archive-azurechina-reference-resources.md#azure-backup-overview)
    * [Recovery Service Vaults](articles/backup-archive-azurechina-reference-resources.md#recovery-service-vaults)
    * [Azure Backup for Files and Folders (MARS Agent)](articles/backup-archive-azurechina-reference-resources.md#azure-backup-for-file)
    * [Azure Backup Server (DPM)](articles/backup-archive-azurechina-reference-resources.md#azure-backup-server)
    * [Azure Backup for IaaS / File Shares](articles/backup-archive-azurechina-reference-resources.md#azure-backup-for-iaas)
    * [Archiving Data on Azure](articles/backup-archive-azurechina-reference-resources.md#archive-data)

## Walkthroughs

 * [Walkthrough: Create Recovery Service Vault](https://docs.microsoft.com/en-us/azure/backup/backup-configure-vault#create-a-recovery-services-vault)
 * [Walkthrough: install and register azure recovery service agent](https://docs.microsoft.com/en-us/azure/backup/backup-configure-vault#install-and-register-the-agent)
 * [Walkthrough: Backup files and Folders](https://docs.microsoft.com/en-us/azure/backup/backup-configure-vault#back-up-your-files-and-folders)
 * [Walkthrough: Deploying Azure Backup Server](https://docs.microsoft.com/en-us/azure/backup/backup-azure-microsoft-azure-backup)
 * [Walkthrough: Backup & Restore Azure VM](https://docs.microsoft.com/en-us/azure/backup/backup-azure-arm-restore-vms)

